class SaliencyScorer:
	...
	'''
	def __init__(self, data, flip_saliency: bool = False):
		"""
		...
		"""
		# Calculate the frequency of each attribute (using flipped attributes so very frequent attributes are considered equivalent to very rare attributes)
		ratio = data.flipped_attributes.sum(0).float() / data.flipped_attributes.size(0)
		# Normalize so that the mean is 1
		ratio /= ratio.mean()
		# Flip saliency scores if necessary
